...
import { getSchemaBaseUrl, abandonStagedSchema } from "@/utils/schema-api";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await abandonStagedSchema(baseUrl, conn.secret);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to abandon schema";
    return NextResponse.json(
      { error: message },
      { status: 502 }
    );
